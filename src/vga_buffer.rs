use crate::vga_buffer::Color::{Green, White};
use core::fmt;
...
lazy_static! {
    pub static ref WRITER: Mutex<Writer> = Mutex::new(Writer {
        column_position: 0,
        color_code: ColorCode::new(White, Green),
    ...
    pub fn write_string(&mut self, s: &str) {
        for byte in s.bytes() {
            match byte {
                ...
                // print white square for unprintable bytes
    ...
    pub fn new_line(&mut self) {
        for row in 1..BUFFER_HEIGHT {
            for col in 0..BUFFER_WIDTH {
                // move all characters one line up
                let old_char = self.buffer.chars[row][col].read();
                self.buffer.chars[row - 1][col].write(old_char);
            }
        }

        // clear bottom row to make space for new line
        self.clear_row(BUFFER_HEIGHT - 1);
        // reset column_position to 0 to start a new line
